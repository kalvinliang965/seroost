...
fn save_model_as_json(model: &Model, index_path: &str) -> Result<(),()> {
    ...
    serde_json::to_writer(BufWriter::new(index_file), &model).map_err(|err| {
        eprintln!("ERROR: could not serialize index in to file {index_path}: {err}");
    })?;
    Ok(())
}
...
fn add_folder_to_model(dir_path: &Path, model: &mut Model) -> Result<(),()> {
    ...
    'next_file: for file in dir {
        let file = file.map_err(|err| {
            eprintln!("ERROR: could not read next file in the directory {dir_path} during indexing: {err}",
                    dir_path=dir_path.display());
        })?;
...
fn usage(program: &str) {
    ...
    eprintln!("     search <index-file> <query>          search <query> within <index-file>");
